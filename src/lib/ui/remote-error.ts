 ...
 * server function deliberately explains (`error(403, '…')`). This reads the message the
 * server chose, and still handles a plain `Error` (a network failure, a client-side throw).
 */
export function remoteErrorMessage(err: unknown, fallback: string): string {
	if (isHttpError(err)) return err.body.message || fallback
