...
import { createEgressProxy, EGRESS_REFUSAL_HEADER, EgressBlockedError, type EgressProxy } from '../src/lib/tools/egress.server'
import {
	browserClose,
...
import { PDF_MAX_BYTES, pdfRead, webFetch } from '../src/lib/tools/web-fetch.server'
...
test.describe('inside the browser, the egress proxy sees every request', () => {
	...
	test('loopback is not exempt from the proxy', async () => {
		// Chromium bypasses a proxy for localhost unless told otherwise; ungated, this would load.
		const page = await browser.newPage()
		try {
