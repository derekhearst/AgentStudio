...
- 2026-05-02 — Phase 7 (context utilization indicator) shipped on branch `claude/nervous-kapitsa-18255e`. The chat stream handler now emits a `context_stats` SSE event right after slot assembly, carrying `{ tokenEstimate, contextWindow, didCompact, includedSlots, droppedSlots, truncatedSlots, systemPromptTokens }`. The chat client captures it into a `liveContextStats` reactive state. `contextMetrics` derivation prefers the live SSE-supplied value over the chars/4-derived client estimate when present, so the existing `ContextWindow` component (already in the toolbar near the model picker) now reflects the tokenizer-accurate count from the server. No new component needed — wired into the existing one.
- 2026-05-02 — Phase 4 (relevance-filtered skills) shipped on branch `claude/nervous-kapitsa-18255e`. New `description_embedding` (vector(1536)) + `description_embedded_at` columns on `skills` (drizzle migration `0020_tense_tiger_shark.sql`). Three new helpers in `skills.server.ts`: `refreshSkillEmbedding(skillId)` recomputes the embedding for one skill (called from `createSkill` and `updateSkill` whenever name/description change), `backfillSkillEmbeddings(limit)` embeds any pending rows in batch, and `listRelevantSkillSummaries(query, topK)` returns the top-K most relevant skills by cosine distance over the persisted vectors with a hard fallback to the full list when embedding fails. Stream handler now calls the relevance helper with the user's content + `contextConfig.skillTopK` setting (default 8), so chats only see the relevant slice of the skill catalog instead of every skill every turn. Bootstrap and the `/api/cron` tick both run `backfillSkillEmbeddings(50)` so newly added or migrated skills get embedded automatically.
- 2026-05-02 — Phase 8 (per-agent slot config) shipped on branch `claude/nervous-kapitsa-18255e`. New `context_slot_configs` table (migration `0021_pale_hiroim.sql`) holds per-(user, agent?, slot) overrides for `tokenBudget`, `priority`, and `enabled`. New `loadSlotOverrides(userId, agentId?)` returns a merged map (agent-specific wins over user-wide), and `applySlotOverrides(slots, overrides)` returns a new slot list with the overrides applied (disabled slots dropped, others have their priority/tokenBudget overridden). Stream handler loads overrides and applies them before `assembleSystemPrompt`. UI follow-up (a "Context" tab on `/agents/[id]/settings`) is left for the agents/UI workstream. **#4 Context is now fully complete — all 8 phases shipped.**
