...
Status: completed (Phases 1-4 landed; Phase 5 container isolation is explicit future-work)
...
- 2026-05-02 — Phase 3 (workspace GC) shipped on branch `claude/nervous-kapitsa-18255e`. New `src/lib/workspace/gc-core.ts` (pure, server-agnostic — takes a `lookupRuns` callback) and `src/lib/workspace/gc.server.ts` (Drizzle-backed wrapper). GC scans `<sandbox>/<userId>/runs/<runId>` dirs, deletes those whose `chat_runs.state` is terminal AND `finished_at` is older than `ttlDays` (default 7). Skips active runs, recently-finished runs, and orphans (no DB row — kept for inspection). Legacy `<sandbox>/<userId>` paths are never touched. Wired into `/api/cron/+server.ts` so it runs on every cron tick alongside `checkAndRunAutomations`. New CLI script `scripts/gc-workspaces.ts` for manual invocation with `--dry-run` and `--ttl-days` flags.
- 2026-05-02 — Phase 2 (persistent workspace mode) shipped on branch `claude/nervous-kapitsa-18255e`. `WorkspaceContext` extended with optional `persistentKey`; `resolveWorkspaceRoot` now picks a path in order: `persistent/<key>/` if `persistentKey` set, else `runs/<runId>/` if `runId` set, else legacy `<userId>/`. Path-traversal rejection extended to the new key. Agents opt in via `agent.config.workspace = { mode: 'persistent', key: 'foo' }` (freeform jsonb — no schema migration needed). Both the chat stream handler and `runInlineSubagent` read the agent's config and pass `{ persistentKey }` to `executeTool`, which threads it through the AsyncLocalStorage so all FS tools resolve to the persistent dir for the run. Two runs of an opted-in agent with the same key write to the SAME directory (data survives across runs). Phases 4 (git worktree), 5 (container isolation) still pending.
- 2026-05-02 — Phase 4 (git-worktree workspace mode) shipped on branch `claude/nervous-kapitsa-18255e`. New pure `src/lib/workspace/worktree-core.ts` (arg builder + porcelain parser, no node:child_process / SvelteKit deps) and `src/lib/workspace/worktree.server.ts` (`ensureWorktree`, `cleanupWorktree`, `listWorktrees`, `detectHeadBranch` with injectable `GitRunner`). `WorkspaceContext` gains `worktree?: { repoPath, baseBranch?, deleteBranchOnCleanup? }`; `resolveWorkspaceRoot` returns `<sandbox>/<userId>/worktrees/<runId>` when worktree+runId present (precedence: persistentKey > worktree > runId > legacy); `ensureWorkspace` dispatches to `ensureWorktree` for that case. `executeTool` threads worktree config through AsyncLocalStorage. Chat stream + sub-agent read `agent.config.workspace.mode === 'worktree'` (with `repoPath`, `baseBranch`, `deleteBranchOnCleanup`). GC core extended to scan `<userId>/worktrees/<runId>` alongside `<userId>/runs/<runId>` (results carry `kind: 'run' | 'worktree'`); a new `removeWorktree(path, runId)` hook deregisters the worktree from its parent repo before the rm fallback (errors surface but never block rm). Three new read-only tools available only when worktree config is set: `git_status`, `git_log`, `git_diff` (with optional `--staged`, `ref`, `paths`). 13 tests cover pure helpers, real git integration, and GC scanning + error tolerance. Phase 5 (container isolation) remains explicit future-work per the spec.
