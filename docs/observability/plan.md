...
- 2026-05-04 — Phase 3 partial (commit 32c68ca). `/review/trace/[runId]` viewer page (admin-gated) with summary card, expandable timeline, linked-records footer. `hook_failure` source wired at hooks-bus dispatch failure path with dedupeKey `hook:<runId>:<hookName>:<event>`. 5 of 9 enum sources active.
- 2026-05-04 — Phase 3 + Phase 5 partial (commit 071be0d). `policy_override_request` source wired from chat-stream budget-block path. DedupeKey `budget:<limitId>:<userId>`. 6 of 9 enum sources active.
- 2026-05-04 — Phase 4 (next commit). `metrics_sample` job + 5min scheduler tick. `recordMetric` / `runMetricsSample` / `listLatestMetrics` / `listMetricTimeseries` in metrics.server.ts. `/review/health` admin dashboard with review-inbox rollup + queue depth + review_inbox + runs metric groups.
