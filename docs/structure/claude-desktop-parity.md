...
| Deep research | **even** | approval-gated plan, background run, cited report. (Until 2026-09-23 every run failed at its first model call — the bare SDK model id was sent to OpenRouter — and a canceled run was retried to completion.) Worse in one way: the loop is a fixed pipeline, so a run cannot be steered mid-flight — only approved or denied up front | agentic, steerable |
