...
| Deep research | **even** | background run with a cited report, started by a research-mode automation; in the chat, the Research agent's approval-gated plan hands off to another agent, which works through it in the chat rather than starting a background run (the composer's Research button that would start one is not shown). (Until 2026-09-23 every run failed at its first model call — the bare SDK model id was sent to OpenRouter — and would have failed again at its last, marking the cited sources; a canceled run was retried to completion, or saved its report over a Cancel pressed during the write-up.) Worse in one way: the background loop is a fixed pipeline, so a run cannot be steered mid-flight | agentic, steerable |
...
**The evens are thinner than they look.** Three of them hide a real disadvantage: a background research run is a fixed pipeline that cannot be steered once started; `run_code` has no chart output; dictation has no live transcript. And the one former "even" that was actually a lie is scheduling — the cron parser rejects `0 9 * * 1-5`, and what it does accept runs on UTC, so a 9am job fires at 3am here.
