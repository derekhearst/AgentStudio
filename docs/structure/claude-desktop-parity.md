...
| Search across conversations | **even** (#18) | server-side full-text search over all history: message text, attachment names and every tool call's paths, commands, short arguments and printed links, so a turn is found by the file it edited or the PR it opened; ranked, with highlighted extracts, prefix matching while typing, archived chats on request | server-side across all history |
