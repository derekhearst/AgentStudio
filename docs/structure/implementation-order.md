...
18. [ ] Research loop domain (search→fetch→synthesize)
    ...
    - Phases 2-5 (research orchestrator skill + loop, research job type via #17 worker, deep-research toggle + per-agent config, report UI with citations + file grounding) still pending. Phase 1 establishes the durable data model + the safest single tool needed; the orchestration layer plugs into the existing #17 jobs queue once the loop driver is written.
