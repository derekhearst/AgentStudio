...
10. [ ] Runtime extraction/composition server
    ...
    - Phases 2 (formal AgentDefinition builder), 3 (Environment descriptor — partially in place via ToolUserContext), 5 (replace inline-subagent loop), 6 (reuse runtime for automations) still pending. Phase 5 is the highest-value follow-up — inline-subagent.ts is a near-duplicate of the same loop and would shed ~400 lines once it routes through `runChatLoop` with a forwarded Session.
