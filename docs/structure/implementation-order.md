...
19. [x] Source-control workflow (branch, diff, PR) **(P1 schema gate met; provider sync + agent tools deferred)**
    ...
    - Phases 2 finish (worktree provisioning from real remotes — clone or reuse local mirror, check out generated branch), Phase 3 finish (`prepare_commit` / `push_branch` / `create_pull_request` agent tools wired through the runtime), Phase 4 (PR review handoff via `/review`), Phase 5 (webhook-driven status sync) remain deferred. The OAuth + repo-discovery foundation is durable enough today to back follow-up work without revisiting the schema or connection model.
...
21. [x] Automations scheduling and trigger framework **(gate met via Wave 4 #17 P5 finish migration)**
    ...
    - Phase 4 (research-mode + repo-aware coding-mode workflow integration via per-mode handlers) and Phase 5 (budget caps + approval gates wired through #20 review inbox) remain deferred. P3 lands the schema + observability so subsequent phases have a clean attachment point.
...
22. [x] Agents prompt-source + identity architecture **(P1 — orchestrator identity as a skill)**
    ...
    - Phase 4 (AGENTS.md repo-file discovery scanner) remains deferred — it's the only phase that requires a file-system walker + frontmatter parser + last-write-wins conflict resolution between repo and DB sources. Worth its own slice once a real repo-driven workflow surfaces.
