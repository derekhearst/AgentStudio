...
10. [ ] Runtime extraction/composition server
    ...
    - Phases 2 (formal AgentDefinition builder), 3 (Environment descriptor), 6 (reuse runtime for automations) still pending — and lower priority. Phases 2/3 are formalizations of patterns that already work inline (slot assembly + workspace context); they'd be cosmetic refactors with no behavior change. Phase 6 is a feature add (automations currently use a single-shot `chat()` call with no tools — running an actual agent loop on a schedule is a separate design decision). The big-payoff loop deduplication is done.
...
11. [ ] Task lifecycle alignment with runtime/runs
    ...
    - Phase 3 (task runner worker — picks pending child tasks → spawns runs) overlaps significantly with the runtime extraction in #10 and is best built on top of that. Phase 5 (retries + DAG visualization) is future-work.
