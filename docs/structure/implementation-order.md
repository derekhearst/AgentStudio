...
15. [x] Project artifacts/versioning and linkage
    ...
    - Phase 2 remainder (`sessions.projectId` / `currentArtifactId` columns + `set_project_context` tool to bind a project to the conversation), Phase 3 (Memory bridge — `memoryDrawers.linkedArtifactId`), Phase 5 (AI UX polish — confident artifact selection, ask-on-ambiguity), and Phase 7 (domain doc) still pending. The agent can now create + edit project artifacts as part of any conversation; binding a project to a session for sticky context is the next slice.
...
17. [ ] Jobs queue/worker reliability and handler manifest
    ...
    - Phase 2 (run-execution handoff to jobs) + Phase 5 remainder (migrate automation ticks) + Phase 6 (web/worker process split) still pending. End-to-end durability now demonstrated across 4 feature paths: research_run (user-initiated), evaluation_run (post-chat), memory_mine (post-chat dedupe), workspace_gc (scheduled daily). Same pattern applies to automations next.
