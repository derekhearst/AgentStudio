...
15. [x] Project artifacts/versioning and linkage
    ...
    - Phase 2 (chat session integration — `sessions.projectId` / `currentArtifactId` + `set_project_context` / `create_artifact` / `edit_artifact` tools), Phase 3 (Memory bridge — `memoryDrawers.linkedArtifactId`), Phase 5 (AI UX polish — confident artifact selection, ask-on-ambiguity), and Phase 7 (domain doc) still pending. The data model is durable + the manual UI surface lets users create / version / rollback artifacts today; the agent integration is the next phase that ties projects to chat sessions.
...
17. [ ] Jobs queue/worker reliability and handler manifest
    ...
    - Evidence (Phase 5 partial — memory_mine job migration + bug-fix for claim query, 2026-05-03):
      ...
      - 4 new tests: dedupeKey collapses repeated enqueue for same conversation, default-queue background priority (50), cross-conversation dedupe keys are independent, run_id back-link to originating chat run: [tests/memory.mine-job.spec.ts](../../tests/memory.mine-job.spec.ts)
    ...
    - Phases 2 + 4 + Phase 5 remainder (run-execution handoff to jobs, cron-like scheduler, migrate evaluations + automations + workspace GC, web/worker process split) still pending. End-to-end cancellation now works for the first feature on the queue (research_run); same pattern can be applied to future migrated handlers.
...
18. [ ] Research loop domain (search→fetch→synthesize)
    ...
    - Phase 4 remainder (per-agent research config — `agents.config.research` enable + default-model overrides) and Phase 5 (inline citation rendering in chat report bubbles, PDF attachment grounding) still pending. The user-facing trigger surface is now end-to-end: type a research query, click the magnifying glass, watch the live trace with status + sources + report.
