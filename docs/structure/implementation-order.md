...
9. [ ] Skills taxonomy and loading rules (including mode identities)
   ...
   - Phases 2 (skills browser/authoring UI), 5 (skill-aware tool-output offload) still pending. (Phase 5 conceptually overlaps with #8 phase 5 which already shipped — the workspace-backed offload from there is the durable side; the "load summary on demand" half still needs a small extension to read_skill for partial-body fetches.)
