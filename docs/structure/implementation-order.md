...
17. [ ] Jobs queue/worker reliability and handler manifest
    ...
    - Phases 2-6 (run-execution handoff to jobs, durable cancellation through runtime, cron-like scheduler, feature migration of automations + evaluations + memory mining, web/worker process split) still pending. The queue primitives are now durable + handler-pluggable; the remaining phases progressively migrate inline work into the jobs system.
