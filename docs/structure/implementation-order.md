...
17. [ ] Jobs queue/worker reliability and handler manifest
    ...
    - Phases 2-4 + Phase 5 remainder (run-execution handoff to jobs, durable cancellation through runtime, cron-like scheduler, migrate evaluations + automations + workspace GC, web/worker process split) still pending. The queue primitives are now durable + handler-pluggable + battle-tested by the memory mining + research_run migrations.
...
18. [ ] Research loop domain (search→fetch→synthesize)
    ...
    - Phases 4-5 (chat composer Deep Research toggle + per-agent research config; report UI rendering with inline citation links + file grounding via PDF attachment) still pending. The end-to-end loop now runs through the jobs queue with full progress tracing in /research; the next slice is the chat-mode integration so users can trigger research from a conversation.
