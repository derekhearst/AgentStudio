...
15. [x] Project artifacts/versioning and linkage **(gate fully met across P1+P2 partial+P7)**
    ...
    - Phase 3 (Memory bridge — `memoryDrawers.linkedArtifactId`) and Phase 5 (AI UX polish — confident artifact selection, ask-on-ambiguity) still pending. Session binding is now end-to-end: an agent can `set_project_context(projectId)` once, and every subsequent `create_artifact` / `edit_artifact` call defaults to the bound project via the system-prompt context slot's hint.
