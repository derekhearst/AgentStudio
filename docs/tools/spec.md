...
- **`browser_screenshot` needs a URL.** Because nothing is left open between calls, there is no "current page" to capture without one. The screenshot is handed to the model as an image it can see, not as text.
