...
- 2026-05-02 — Phase 2 (tool-usage ledger) shipped on branch `claude/nervous-kapitsa-18255e`. New `tool_usage` table mirroring `llm_usage`'s context FKs (user/run/agent/task) plus `tool_name`, optional `provider`, `unit_type` (credit/second/call/mb), `units`, `cost`, freeform metadata, and indexes for all the rollup paths. New `logToolUsage(input)` helper accepts either a direct `cost` or computes from `units * costPerUnit`. `getCostSummary` extended with `toolSpend`, `toolCallCount`, `byTool` (per-tool/provider/unit-type breakdown), and a `combinedSpend` total = LLM + tool. **`web_search` tool is now instrumented**: every call writes a `tool_usage` row with `unit_type='call'`, `units=1`, `provider='searxng'` when SEARXNG_URL is set, and `cost = SEARCH_COST_PER_CALL_USD env (defaults to 0 since SearXNG is self-hosted)`. Other tool wrappers (browser, code_exec) are not instrumented yet — opt-in per-tool when their per-unit costs are known.
- 2026-05-03 — Phase 3 (budget limits + alerts) shipped on branch `claude/nervous-kapitsa-18255e`. Three new pgEnums (`budget_scope`, `budget_period`, `budget_action`, `budget_trigger_type`) plus `budget_limits` table (scope/scopeId/period/limitUsd/warnUsd/action/enabled, scoped per user) and append-only `budget_alerts` table (cascades on parent limit delete). New `src/lib/costs/budget.server.ts` exports `checkBudgetLimits(ctx)` which sums LLM+tool spend over the period for every applicable enabled limit and returns `{ allowed, blockedBy, warnings }`, and `recordBudgetAlert(input)` which is per-period idempotent (one alert row per limit/trigger/period window). The chat stream handler runs `checkBudgetLimits` BEFORE the chat_run insert — `block` actions return HTTP 402 with `error: 'budget_exceeded'` and don't create an orphan run; `notify_only` writes a warn alert and lets the run proceed. Periods supported: day (UTC midnight), week (Sunday boundary), month (first of month). Scopes: global, agent, run (project scope reserved for #15). Notification delivery is intentionally not wired in this slice — alerts are persisted; the notifications domain can read them later.
- 2026-05-03 — Phase 4 (dashboard improvements) shipped on branch `claude/nervous-kapitsa-18255e`. The `/cost` page gains four new panels driven by the rollups added in Phases 1-3: combined LLM + tool spend (showing all four numbers — total, LLM, tool, tool-call count), Top Runs by Cost (linkable label/state/source/tokens), Top Agents by Cost (links to `/agents/[id]`), and Tool Spend (per-tool/provider/unit-type table). Five new remote functions for budget CRUD: `listBudgetLimits`, `createBudgetLimit`, `updateBudgetLimit`, `deleteBudgetLimit`, and `listBudgetAlerts` — wired into `cost.remote.ts` with Zod schemas, ready for a follow-up UI slice that surfaces budget-limits-as-progress-bars + alert history (the data path is now fully present; the markup follow-up is small). Phase 5 (provider reconciliation against OpenRouter invoices) still pending — explicitly marked as future-work in the spec.
