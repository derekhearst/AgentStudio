...
| `run_automation`     | Runs one of the owner's own automations once, as a manual run (see Business rules)                                |
...
- A monitor-fired automation run counts as a **manual run**, like pressing "Run now": it does not move the automation's next scheduled time, it runs even if the automation's schedule is switched off, it is not retried on failure, and a failure does not count toward the automation's auto-disable streak.
