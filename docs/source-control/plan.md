...
Status: active (Phases 1-5 complete; only deeper integrations like clone-on-attach + push-without-confirmation remain as follow-ups)
...
- 2026-09-22 — git execution hardening — every server-side git call now goes through one runner, [src/lib/source-control/git-exec.server.ts](../../src/lib/source-control/git-exec.server.ts) (pure half in [git-exec.ts](../../src/lib/source-control/git-exec.ts)): host git config off (`GIT_CONFIG_NOSYSTEM`, `GIT_CONFIG_GLOBAL=/dev/null`), an environment allowlist, `-c` overrides for every program-running key (hooks, fsmonitor, askpass, credential helpers, alternate-refs command, signing), the filter drivers of the repo and of every checked-out submodule (found via `ls-files --stage` gitlinks, followed recursively) neutralised by name, with the scan failing closed (`GitRefusedError`) on an unparseable config, a timeout or the limits in `GIT_SCAN_LIMITS`, `--no-ext-diff`/`--no-textconv`/`--submodule=short`/`--ignore-submodules=dirty` plus `diff.submodule=short`/`status.submoduleSummary=false`, a pinned `--work-tree`, `GIT_ALLOW_PROTOCOL=https`, and the token as a URL-scoped `Authorization` header via `GIT_CONFIG_COUNT` with that URL's proxy/sslVerify/curloptResolve/cookieFile/saveCookies pinned (the `$GIT_TOKEN` credential helper is gone). Remote calls refuse a repo that sets `url.*.insteadOf`/`pushInsteadOf`, `remote.<url>.*` or any URL-scoped `http.*`. Same change: `git_diff` refs validated and placed after `--end-of-options`; mirror/project refresh fetches `+refs/heads/*:refs/remotes/origin/*` and fast-forwards the checked-out branch (it used to write only FETCH_HEAD); `--force-with-lease` names its expected commit — `origin/<branch>` when origin is the target, else `refs/agentstudio/pushed/<target>/<branch>`, both updated after each push — instead of failing every existing branch with "stale info"; webhook status-irrelevant actions keep the PR's status and merge metadata; webhook repo lookup is case-insensitive and GitHub-only; transient GitHub failures (including header-less secondary-limit 403s) no longer disable the connection; "connect GitHub" errors point at `/projects`. Residual: the scan and the command are two steps, so an agent rewriting `.git/config` (or checking out a submodule) in that window could race it — closing it means running server-side git inside bubblewrap. Specs: [tests/source-control.git-exec.spec.ts](../../tests/source-control.git-exec.spec.ts), [git-tools](../../tests/source-control.git-tools.spec.ts), [repo-mirror](../../tests/source-control.repo-mirror.spec.ts), [write-tools](../../tests/source-control.write-tools.spec.ts), [webhook-pr-state](../../tests/source-control.webhook-pr-state.spec.ts), [github-errors](../../tests/source-control.github-errors.spec.ts), with a local HTTP git remote in [tests/git-http-server.ts](../../tests/git-http-server.ts).
- 2026-05-04 — Wave 5 #19 phase 2 finish (runtime integration) — `tasks.repository_id` column ([drizzle/0044_task_repository_id.sql](../../drizzle/0044_task_repository_id.sql)) lets an operator link a task to a connected repo; `provisionRepoBackedWorkspace` ([src/lib/source-control/repo-worktree.server.ts](../../src/lib/source-control/repo-worktree.server.ts)) composes mirror materialization + branch naming into one call the task-runner invokes when the column is set. `WorktreeConfig.branch` is now plumbed through the worktree pipeline so per-task branches (`agent/<taskId>` first attempt, `agent/<taskId>/attempt-<N>` on retry) replace the legacy `run/<runId>` shape for repo-backed runs. Failure is non-fatal — the runner falls back to the agent's legacy workspace + logs a warning. End-to-end: operator creates task → runner clones-or-fetches mirror → checks out per-attempt branch → agent uses prepare_commit / push_branch / create_pull_request → review-inbox handoff. 5 contract tests in [tests/source-control.repo-worktree.spec.ts](../../tests/source-control.repo-worktree.spec.ts).
