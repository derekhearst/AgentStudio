...
- 2026-09-22 — git execution hardening — every server-side git call now goes through one runner, [src/lib/source-control/git-exec.server.ts](../../src/lib/source-control/git-exec.server.ts) (pure half in [git-exec.ts](../../src/lib/source-control/git-exec.ts)): host git config off (`GIT_CONFIG_NOSYSTEM`, `GIT_CONFIG_GLOBAL=/dev/null`), an environment allowlist, `-c` overrides for every program-running key (hooks, fsmonitor, askpass, credential helpers, alternate-refs command, signing), the repo's filter drivers neutralised by name, `--no-ext-diff`/`--no-textconv`/`--ignore-submodules=dirty`, a pinned `--work-tree`, `GIT_ALLOW_PROTOCOL=https`, and the token as a URL-scoped `Authorization` header via `GIT_CONFIG_COUNT` (the `$GIT_TOKEN` credential helper is gone). Same change: `git_diff` refs validated and placed after `--end-of-options`; mirror/project refresh fetches `+refs/heads/*:refs/remotes/origin/*` and fast-forwards the checked-out branch (it used to write only FETCH_HEAD); `--force-with-lease` names its expected commit (`origin/<branch>`, updated after each push) instead of failing every existing branch with "stale info"; webhook status-irrelevant actions keep the PR's status and merge metadata; webhook repo lookup is case-insensitive and GitHub-only; transient GitHub failures no longer disable the connection; "connect GitHub" errors point at `/projects`. Residual: filter drivers are neutralised by name after a config read, so an agent rewriting `.git/config` in that window could race it — closing it means running server-side git inside bubblewrap. Specs: [tests/source-control.git-exec.spec.ts](../../tests/source-control.git-exec.spec.ts), [git-tools](../../tests/source-control.git-tools.spec.ts), [repo-mirror](../../tests/source-control.repo-mirror.spec.ts), [write-tools](../../tests/source-control.write-tools.spec.ts), [webhook-pr-state](../../tests/source-control.webhook-pr-state.spec.ts), [github-errors](../../tests/source-control.github-errors.spec.ts), with a local HTTP git remote in [tests/git-http-server.ts](../../tests/git-http-server.ts).
